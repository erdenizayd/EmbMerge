...
class StandardModel(nn.Module):
    def __init__(self, config):
        ...
        self.scoring_layer = nn.Linear(self.hidden_size, 1)
    ...
    def forward(self, lists):
        ...
        for lst_key in lists:
            ...
            score_means = score_embs.mean(dim=1).unsqueeze(1).expand(-1, 100, -1)
            final_embs = torch.cat((model_embs, item_embs,score_embs, rank_embs), dim=2)

            out = self.encoder(final_embs)
            out_mean = out.mean(dim=1)

            out_mean_expanded = out_mean.unsqueeze(1).expand(-1, 100, -1)
            alpha = torch.sigmoid(self.output_layer(out_mean))

            scaled_scores = self.score_scale * scores
            scaled_alpha  = self.alpha_scale * alpha

            final_scores = scaled_scores * scaled_alpha 
